...
import {checkProperty} from "../../utils/consts/functions"
		...
		<Stack gap={2}>
			<div
				...
				<label htmlFor={"Sche" + items[0].meatNumber}>
					<AgingEditCardHeader item={items[0]} show={show} setShow={setShow} />
				</label>
			</div>
			{show &&
				items.map((item) => {
...
const NewAgingCardItem = (props: INewAgingCardItem) => {
	...
	const onClickDeleteButton = (item: MeatInfoWithEntry) => {
		const ok = window.confirm("입고 중인 아이템입니다. 정말 삭제하시겠습니까?")
		if (ok) {
			setTimeout(() => {
				const rOk = window.confirm("복구 안됩니다. 진짜진짜삭제합니까??")
				if (rOk) {
	...
	const onCheckElement = (checked: boolean, itemId: string) => {
		console.log(item)

		console.log(isEmpty())
		if (checked) {
			setCheckedSList([...checkedSList, itemId])
		} else {
			const newList = _.without(checkedSList, itemId)
	...
	const isEmpty = () => {
		const a =
			checkProperty(item, "agingDate") ||
			checkProperty(item, "beforeWeight") ||
			checkProperty(item, "fridgeName") ||
			checkProperty(item, "floor") ||
			checkProperty(item, "ultraTime")
					...
					<div
						...
						{isEditMode ? (
							<div
								...
								<TiDeleteOutline onClick={() => onClickDeleteButton(item)} />
							</div>
						) : (
							<></>
						)}
